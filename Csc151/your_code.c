...
void GenerateILOC(ASTNode* node)
{
	switch(node -> type){
		...
		case ASTNODE_IDENT:
			if(node -> offset == NULL)
				node -> offset = SearchOffset(node->name);
			node->regNum = GetNextRegister();
		...
		case ASTNODE_ASSIGN:
			GenerateILOC(node -> right);
			if(node -> left -> offset == NULL)
				node -> left -> offset = SearchOffset(node->left->name);
...
void Emit(ASTOp operation, char *source_1, char *source_2, char *destination){
	...
	/*if (operation == LABEL) {
		//fprintf(fp, "\n%s: ", source_1);
		printf("\n%s: ", source_1);
	}else if (source_2 == NULL && destination == NULL) {
		//fprintf(fp, "%s -> %s\n ", opcode, source_1);
		printf("%s -> %s\n", opcode, source_1);
